...
const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({
  ...
  const selectedTags = showTags && isMulti ? selectedValues.map(value => {
  ...
  return (
    <div className={`relative w-full ${className}`} ref={dropdownRef}>
      ...
      <div
        ...
        className={`flex items-center justify-between w-full px-4 py-2 bg-white border rounded-md 
          ${disabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : 'cursor-pointer'}
          ${error ? 'border-red-500' : 'border-gray-300'}
          ${!disabled && 'hover:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent'}`}
      >
      ...
      {error && (
        <p className="mt-1 text-sm text-red-500" id={`${uniqueId.current}-error`}>
          {error}
        </p>
      )}
            ...
            options.map((option) => (
              ...
              >
                <div className="mr-3 flex items-center">
                  <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors duration-200 ease-in-out ${
                    selectedValues.includes(option.value) 
                      ? 'border-emerald-600' 
                      : 'border-gray-300'
                  }`}>
                    <div className={`w-2.5 h-2.5 rounded-full transform transition-all duration-200 ease-in-out ${
                      selectedValues.includes(option.value)
