...
export default function InterventionsPage() {
  ...
  const [archivingIntervention, setArchivingIntervention] = useState<Intervention | null>(null);
  ...
  const [interventions, setInterventions] = useState<Intervention[]>([
    {
      ...
      type: 'Custom',
      title: 'Coaching',
      description: 'This teacher receives regular coaching from the school leadership team or an external partner.',
      isArchived: false,
  ...
  const handleArchiveConfirm = () => {
    if (archivingIntervention) {
      ...
      console.log('Toggling archive state for intervention:', updatedIntervention);
  ...
  const handleSave = (updatedIntervention: Omit<Intervention, 'id'>) => {
    // Here you would typically make an API call to update the intervention
    console.log('Saving intervention:', { ...updatedIntervention, id: editingIntervention?.id });
  ...
  const filteredInterventions = interventions
    .filter((intervention) => {
      const matchesSearch = intervention.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        intervention.description.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesType = filterType === 'Both' ? true : intervention.type === filterType;
  ...
  const filterCounts = {
    Default: interventions.filter(i => i.type === 'Default' && i.isArchived === isActive).length,
    Custom: interventions.filter(i => i.type === 'Custom' && i.isArchived === isActive).length,
