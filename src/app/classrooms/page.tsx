...
export default function ClassroomsPage() {
  ...
  const handleSave = async (updatedRow: any) => {
    try {
      let data = {
        school_name: updatedRow.school,
        course: updatedRow.course,
        teacher_name: updatedRow.teacher,
        grades: updatedRow.grades,
        class_section: updatedRow.classPeriod,
        interventions: updatedRow.tags.map((tag: { id: string }) => tag.id),
        curriculums: updatedRow.instructionalMaterials.map(
          (material: { id: string }) => material.id
        ),
      };
      const response = await editClassroom(updatedRow.id, data);
  ...
  const handleSelectRow = (
    ...
    const newSelected = {
      ...
      classes: new Set(selectedRows.classes),
    };

    const school = allClassrooms.find((s) => s.id === schoolId);
    if (!school) return;
  ...
  const handleSelectAll = (): void => {
    ...
    if (!selectAll) {
      ...
      allClassrooms.forEach((school) => {
        ...
        school.classes.forEach((classroom) => {
          newSelected.classes.add(`${school.id}-${classroom.id}`);
        });
      });
    }
  ...
  const updateSelectAllState = (selected: typeof selectedRows): void => {
    ...
    allClassrooms.forEach((school) => {
      ...
      selectedClassrooms += school.classes.filter((classroom) =>
        selected.classes.has(`${school.id}-${classroom.id}`)
      ).length;
    });
  ...
  const handleRestore = (): void => {
    ...
    const newActiveSchools: School[] = [...activeClassrooms];
    const newArchivedSchools: School[] = [...archivedClassrooms];
    ...
    selectedRows.classes.forEach((classroomId) => {
      const [schoolId, roomId] = classroomId.split("-");
      ...
      const selectedClassroomsFromSchool = Array.from(selectedRows.classes)
      ...
      selectedClassroomsFromSchool.forEach((classroomId) => {
        const classroomIndex = archivedSchool.classes.findIndex(
          (c) => c.id === classroomId
        );
        if (classroomIndex !== -1) {
          const [classroom] = archivedSchool.classes.splice(classroomIndex, 1);
          activeSchool!.classes.push(classroom);
        }
      });

      // Remove archived school if it's empty
      if (archivedSchool.classes.length === 0) {
    ...
    activeClassrooms.splice(0, activeClassrooms.length, ...newActiveSchools);
    archivedClassrooms.splice(
      0,
  ...
  const handleArchive = (): void => {
    ...
    selectedRows.classes.forEach((classroomId) => {
      ...
      if (!selectedRows.schools.has(schoolId)) {
        ...
        const classroomIndex = activeSchool.classes.findIndex(
          (c) => c.id === roomId
        );

        if (classroomIndex !== -1) {
          const [classroom] = activeSchool.classes.splice(classroomIndex, 1);
          ...
          archivedSchool.classes.push(classroom);
        }
      }
    });
  ...
  const handleDelete = async () => {
    const selectedSchoolsRows = Array.from(selectedRows.schools);
    const selectedClassesIds = Array.from(selectedRows.classes).filter((id) => {
      const [schoolId] = id.split("-");
      return !selectedRows.schools.has(schoolId);
    });

    // Prepare final ID list to send to backend
    const idsToArchive = [
      ...selectedSchoolsRows,
      ...selectedClassesIds.map((id) => id.split("-")[1]),
    ];

    // Call the API

    const response = await deleteClassroom({ ids: idsToArchive });
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-xl w-full mx-4 transform transition-all duration-300 ease-in-out">
            ...
            {/* Warning Box */}
            <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-6 mt-[10px]">
              <div className="flex items-start">
                <div className="flex-shrink-0">
                  <svg
                    className="h-5 w-5 text-[#C23E19]"
