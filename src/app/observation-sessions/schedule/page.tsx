...
// import ObservationToolStep from "@/components/observation/steps/ObservationToolStep";
...
import { getObservers, getUser } from "@/services/userService";
...
export default function ScheduleObservationPage() {
  ...
  const [selectedObservationTool, setSelectedObservationTool] = useState("");
  ...
  useEffect(() => {
    fetchSchools();
    fetchObservationTools(); // Add this line
  }, []);
  useEffect(() => {
    fetchClassroomsData();
    fetchUsers();
  ...
  // Improved function to fetch observation tools
      ...
      if (response.data && response.data.observation_tools) {
        ...
        const formattedTools = response.data.observation_tools.map(
          (tool: any) => ({
            ...
            created_by: `${tool.created_by_first_name || ""} ${
              tool.created_by_last_name || ""
            }`.trim(),
          })
        );
        ...
        if (formattedTools.length > 0) {
          setSelectedObservationTool(formattedTools[0].id);
        }
      } else {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await getObservers(selectedSchool);
      const formattedUsers = response.data.observers.map((user: any) => ({
        id: user.id,
  ...
  const getReviewData = () => {
    ...
    // Look up the observation tool name from the ID
    const selectedTool = observationTools.find(
      (tool) => tool.id === selectedObservationTool
    );
    ...
    return {
      ...
      observationTool: selectedObservationTool, // Keep the ID for form submission
      observationToolName: selectedTool
        ? selectedTool.name
        : "No tool selected", // Add tool name for display
  ...
  const handleSchedule = async () => {
    ...
    const formattedData = {
      ...
      observation_tool: selectedObservationTool, // Use the ID for the API
      users: reviewData.users.map((user) => user.id),
      session_admin: reviewData.sessionAdmin.id,
    };

    console.log("Formatted data for API:", reviewData);
      ...
      <AnimatedContainer variant="fade" staggerItems={true} custom={direction}>
        ...
        {currentStep === 1 && (
          <SchoolClassroomStep
            ...
            observationTools={observationTools} // Add this
            selectedObservationTool={selectedObservationTool} // Add this
            onObservationToolChange={setSelectedObservationTool} // Add this
